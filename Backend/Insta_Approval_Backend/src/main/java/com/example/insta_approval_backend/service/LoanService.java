...
public class LoanService {
    ...
    //private static final BigDecimal MIN_LOAN_AMOUNT = new BigDecimal("10000");
    ...
    public LoanApplication applyLoan(Long customerId, LoanApplicationRequest req) {
    	 ...
    	 /*if (req.getLoanAmount().compareTo(MAX_LOAN_AMOUNT) > 0) {
             throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
                 "Loan amount cannot exceed ₹5,00,000. Requested: ₹" + req.getLoanAmount());
         }

         if (req.getLoanAmount().compareTo(BigDecimal.ZERO) <= 0) {
             throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
    ...
    public CustomerResponse getCustomerById(Long customerId) {
    	int randomCibil = ThreadLocalRandom.current().nextInt(650, 851);
        Customer customer = customerRepo.findById(customerId)
        ...
        response.setCibilScore(randomCibil);
